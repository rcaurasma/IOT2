...
## Endpoints
- **POST** `/auth/register` → { name, last_name, email, password }
- **POST** `/auth/login` → { email, password } → { token, user }
- **GET** `/users` → lista todos los usuarios (id, name, last_name, email, created_at)
- **GET** `/users/:id` → obtiene un usuario por id
- **POST** `/users` → { name, last_name, email, password }
- **PUT** `/users/:id` → { name, last_name, email }
- **DELETE** `/users/:id`
